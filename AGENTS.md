...
This document summarizes common development practices for the LeafLogic project.
...
- **`.codex/`** (inside each service directory, e.g., `WebUI/.codex/`, `Rest-Servers/.codex/`):
  ...
  - `prompts/`: LLM prompts and model configuration docs. Update alongside code and follow **Prompter Mode** guidance.
...
- For Android modules, use Android Studio with the Gradle wrapper and run `./gradlew lint test` before committing.
